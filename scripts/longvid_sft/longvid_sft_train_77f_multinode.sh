#!/bin/bash
# Launch Wan 2.1 1.3B training on Sekai (77-frame) across multiple nodes
# Usage: RANK=<node_rank> WORLD_SIZE=<num_nodes> bash scripts/longvid_sft/longvid_sft_train_77f_multinode.sh
# Example (8 nodes, 64 GPUs total):
#   Node 0: RANK=0 WORLD_SIZE=8 bash scripts/longvid_sft/longvid_sft_train_77f_multinode.sh
#   Node 1: RANK=1 WORLD_SIZE=8 bash scripts/longvid_sft/longvid_sft_train_77f_multinode.sh
#   ...
...
CONFIG="examples/train/configs/longvid_sft/finetune_wan2.1_longvid_sft_1.3B_77f.yaml"
COORD_DIR="/pfs_root/sc/torch_coord"
CONFIG_NAME="longvid_sft_wan2.1_1.3B_77f"

#!/bin/bash
# Launch Wan 2.1 1.3B training on Sekai (161-frame, ~10s) across multiple nodes
# Stage 2 of curriculum learning: resume from 77f checkpoint
# Usage: RANK=<node_rank> WORLD_SIZE=<num_nodes> [RESUME_FROM=<ckpt_dir>] bash scripts/longvid_sft/longvid_sft_train_161f_multinode.sh
# Example (8 nodes, 64 GPUs total):
#   Node 0: RANK=0 WORLD_SIZE=8 RESUME_FROM=/pfs_root/sc/outputs/longvid_sft_wan2.1_1.3B_77f_8x8_20250101_120000 bash scripts/longvid_sft/longvid_sft_train_161f_multinode.sh
#   Node 1: RANK=1 WORLD_SIZE=8 RESUME_FROM=/pfs_root/sc/outputs/longvid_sft_wan2.1_1.3B_77f_8x8_20250101_120000 bash scripts/longvid_sft/longvid_sft_train_161f_multinode.sh
...
CONFIG_NAME="longvid_sft_wan2.1_1.3B_161f"
